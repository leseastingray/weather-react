...
// top level (super) component, contains all others
// makes AJAX call, has weather information, passes it to other components
class App extends Component {
  constructor (props) {
    super(props);
    // instance variable holding the state of the component
    this.state = {
      timezoneOffset: -7, 
      zipcode: "",
      city: {},
    ...
    this.apikey = "&units=imperial&appid="+ process.env.REACT_APP_WEATHERKEY;
    // MY info! weather. process.env refers to .env file, which is a safe place for keys
    this.googleApiKey = process.env.REACT_APP_GMAPKEY; 
  ...
  // NOTE: this is NOT jsx. since we are not in the render
  // method, we use non-jsx stuff like ` and $
  onFormSubmit(zipcode) {
    alert(this.url + zipcode + this.apikey);
    fetch(`${this.url}${zipcode}${this.apikey}`)
    .then(response => response.json())
      .then(data => { 
          const {city, list: forecast } = data; 
          alert(`${this.googleMapsUrl}
    ...
    // takes zipcode input  and sets state with it  
  ...
  // this is where the other page components will go.
  render() {
